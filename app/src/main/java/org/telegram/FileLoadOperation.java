...
public class FileLoadOperation {
    ...
    public static final String TAG = "TGFileLoadOperation";
    ...
    public boolean start() {
        ...
        if (!cacheFileFinal.exists()) {
            ...
            if (BuildConfig.DEBUG) {
                Log.d(TAG ,"start loading file to temp = " + cacheFileTemp + " final = " + cacheFileFinal);
            }
    ...
    private void onFinishLoadingFile(final boolean increment) throws Exception {
        ...
        if (cacheFileTemp != null) {
            ...
            if (!renameResult) {
                if (BuildConfig.DEBUG) {
                    Log.e(TAG, "unable to rename temp = " + cacheFileTemp + " to final = " + cacheFileFinal + " retry = " + renameRetryCount);
                }
                renameRetryCount++;
        ...
        if (BuildConfig.DEBUG) {
            Log.e(TAG, "finished downloading file to " + cacheFileFinal);
        }
    ...
    private boolean processRequestResult(RequestInfo requestInfo, TLRPC.TL_error error) {
        ...
        if (error == null) {
                ...
                if (isCdn) {
                    ...
                    if (cdnCheckPart != lastCheckedCdnPart || finishedDownloading) {
                        ...
                        if (!Arrays.equals(sha256, hash.hash)) {
                            if (location != null) {
                                Log.e(TAG, "invalid cdn hash " + location + " id = " + location.id + " local_id = " + location.local_id + " access_hash = " + location.access_hash + " volume_id = " + location.volume_id + " secret = " + location.secret);
                            } else if (webLocation != null) {
                                Log.e(TAG, "invalid cdn hash  " + webLocation + " id = " + webLocation.url + " access_hash = " + webLocation.access_hash);
                            }
                            onFail(false, 0);
                            cacheFileTemp.delete();
                            return false;
            ...
            if (error.text.contains("FILE_MIGRATE_")) {
                ...
                if (location != null) {
                    Log.e(TAG, "" + location + " id = " + location.id + " local_id = " + location.local_id + " access_hash = " + location.access_hash + " volume_id = " + location.volume_id + " secret = " + location.secret);
                } else if (webLocation != null) {
                    Log.e(TAG, "" + webLocation + " id = " + webLocation.url + " access_hash = " + webLocation.access_hash);
                }
                onFail(false, 0);
            }
        }
